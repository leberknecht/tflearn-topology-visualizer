...
class NetworkImageCreator:
    ...
    def generate_svg(self, model, hidden_layer_prefix='dense', output_layer_name='output'):
        ...
        self.known_layer_names = list(map(lambda x: hidden_layer_prefix + str(x), range(self.hidden_layer_count))) + \
                                 [output_layer_name]
        self.set_max_values(model)
        ...
        with model.session.as_default():
            ...
            for i in range(target_node_count):
                ...
                if len(layer_vars) > 1:
                    ...
                    bias_val = bias_values[i]
                    bias_val = max(bias_val, bias_val * -1)
    ...
    def set_max_values(self, model):
        for layer_name in self.known_layer_names:
            ...
            for row_index in range(len(weights)):
                self.max_node_count = max([self.max_node_count, len(weights[row_index])])
                self.max_weight_percentage = float(max([self.max_weight_percentage, max(weights[row_index])]))
                self.min_weight_percentage = float(min([self.min_weight_percentage, min(weights[row_index])]))
            with model.session.as_default():
                if len(layer_vars) > 1:
                    bias_values = tflearn.variables.get_value(layer_vars[1])
                    self.min_bias_percentage = float(min((self.min_bias_percentage, min(bias_values))))
                    self.max_bias_percentage = float(max((self.max_bias_percentage, max(bias_values))))
